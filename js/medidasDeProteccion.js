...
function loadResoluciones (idInvolucrado, idMedida, idEnlace) 
{
	cont = document.getElementById('contResoluciones');
	ajax = objetoAjax();
	ajax.open("POST", "format/medidasDeProteccion/templates/contResoluciones.php");
	...
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("&idInvolucrado=" + idInvolucrado);
}
...
function saveDatosResoluciones(idEnlace, idMedida, fechaConclusion, medidasAplicadas, nuc, tempPrevia, medidaVictima) {
	...
	var idInvolucrado = document.getElementById('idInvolucrado').value;

	console.log('idInvolucrao: ' + idInvolucrado);
    ...
    if (datosValidos) {
        for (var key in acciones) {
            if (acciones[key].valor === '1') {
                acciones[key].funcion(idEnlace, idInvolucrado, idMedida, acciones[key].valor, medidasAplicadas, fechaConclusion, nuc, tempPrevia, medidaVictima);
...
async function saveRatificacion(idEnlace, idInvolucrado, idMedida, ratificacion) {
    const ratificaciones = await obtenerRatificaciones(idInvolucrado); // Esperamos la respuesta
    ...
    if (ratificaciones.length < 1) {
        if(ratificaciones == 0){
			...
			$.ajax({
				...
				data: "&idInvolucrado=" + idInvolucrado + "&idMedida=" + idMedida + "&observacion=" + observacion,
				success: function (resp) {
					var json = resp;
					var obj = eval("(" + json + ")");
					if (obj.first == "NO") {
...
async function saveRevocada(idEnlace, idInvolucrado, idMedida, revocada){
	const revocadas = await obtenerRevocadas(idInvolucrado);
	if(revocadas.length < 1) {		
		...
		$.ajax({
			...
			data: "&idInvolucrado=" + idInvolucrado + "&idMedida=" + idMedida + "&observacion=" + observacion + "&fecha=" + fecha,
			success: function (resp) {
				var json = resp;
				var obj = eval("(" + json + ")");
				if (obj.first == "NO") {
					...
					if (obj.first == "SI") {
						var obj = eval("(" + json + ")");
						swal("", "Resoluciones registradas exitosamente. ", "success");
						updateFechaConclusion(idInvolucrado, fecha);
						modalDatosMedida(1, idEnlace, 0, 0, obj.idMedidaUltimo, 'resolucion');
					}
				}
			},
...
function updateConclusion (temporalidad, tipo)
{
	var fechaInicioN = document.getElementById('fechaInicio' + tipo + 'N').value;
	var fechaInicioA = document.getElementById('fechaInicio' + tipo).value;	
	
	if (fechaInicioN == fechaInicioA) 
	{
		var fechaConclusion = document.getElementById('fechaConclusion' + tipo).value;
		var fecha = new Date(fechaConclusion);
	}
	else
	{
		var fecha = new Date(fechaInicioN);
	}
	var nuevaFecha = new Date(fecha.getTime() + (temporalidad * 86400000));
	nuevaFecha = formatDateCalendar(nuevaFecha);
	document.getElementById('fechaConclusion' + tipo + 'N').value = nuevaFecha;
}

// Función para obtener las ratificaciones usando async/await
async function obtenerRatificaciones(idInvolucrado) {    
    try {
...
async function obtenerRevocadas(idInvolucrado){
	try{
		const resp = await $.ajax({
			type: "POST",
			dataType: 'json',
...
//Funcion para actualizar la Fecha de Conclusion
function updateFechaConclusion(idInvolucrado, fechaConclusion){
	$.ajax({
		type: "POST",
		dataType: 'html',
		url: "format/medidasDeProteccion/inserts/resoluciones/updateFechaConclusion.php",
		data: "&idInvolucrado=" + idInvolucrado + "&fechaConclusion=" + fechaConclusion,
		success: function (resp) {
			var json = resp;
			var obj = eval("(" + json + ")");
			if (obj.first == "NO") {				
...
function deleteResolucion(idResolucion, idMedida, idEnlace, tipoResolucion) {	
    swal({
    ...
    function (isConfirm) {
        if (isConfirm) {
            $.ajax({
                ...
                data: "&idMedida=" + idMedida + "&idResolucion=" + idResolucion + "&tipoResolucion=" + tipoResolucion,
                success: function (resp) {
                    try {
...
function modalMedidas(idEnlace, idMedida, nuc, medidaVictima, idInvolucrado, idCont) {
    console.log("IDEnlace: " + idEnlace + " IDMedida: " + idMedida + " NUC: " + nuc + " Medida Victima: " + medidaVictima + " IDInvolucrado: " + idInvolucrado);
    ...
    ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    ajax.send("&idEnlace=" + idEnlace + "&idMedida=" + idMedida + "&nuc=" + nuc + "&medidaVictima=" + medidaVictima + "&idInvolucrado=" + idInvolucrado);
}
...
function closeModalMDP(anio, idEnlace, camMes, rolUser, idCoorporacion) {
	...
	$.ajax({
		...
		success: function (resp) {
			...
			if(rolUser == 4){
				//Solo el coordinador podra verificar cuales tiene pendientes.
				checkMedidas(idEnlace);
			}
		}
	});
}

function checkMedidas (idEnlace) {
	var contAlert = document.getElementById('contAlert');
	ajax = objetoAjax();
	ajax.open("POST", "format/medidasDeProteccion/templates/template_alert.php");
	ajax.onreadystatechange = function () {
		if (ajax.readyState == 4 && ajax.status == 200) {
			contAlert.innerHTML = ajax.responseText;
		}
	}
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
